...
export default function EditModal({ isOpen, onClose, onSave, entry }: EditModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isSell) {
      const price = formData.salePrice || formData.purchasePrice;
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  {isSell ? "Total Sale Proceeds (USD)" : "Amount Invested (USD)"}
                </label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  {isSell ? "Sale Price (Per Share USD)" : "Purchase Price (Per Share USD)"}
                </label>
                <input
                  type="number"
                  step="0.01"
                  value={isSell ? (formData.salePrice || formData.purchasePrice || "") : (formData.purchasePrice || "")}
                  onChange={(e) => {
